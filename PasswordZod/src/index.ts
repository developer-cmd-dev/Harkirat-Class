...
//    @ts-ignore
   const comparePass:boolean =await bcrypt.compare(password,response.password);
...
app.post('/api/v1/signup', async (req, res) => {
    ...
    const requiredBody = z.object({
        ...
        password:z.string()
            ...
            .max(8,{message:"Max legnth is 8"})
            .refine((password)=>/[A-Z]/.test(password),{message:"Password must contain Uppercase Letter"})
            .refine((password)=>/[a-z]/.test(password),{message:"Password must contain Lowercase Letter"})
            .refine((password)=>/[0-9]/.test(password),{message:"Password must contain number"})
    ...
    if (!parsedData.success) {
        res.status(400).json({message:"Incorrect format",error:z.treeifyError(parsedData.error).properties})
        return
    }
